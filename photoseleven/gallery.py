...
from photoseleven.error import only_multimedia_content, responce_fail, responce_success
import os
...
def media(file_id):
    ...
    def post_multimedia(filename):
        if len(filename) < 3 or filename[-3:] not in current_app.config['ALLOWED_MEDIA_EXT']:
            return responce_fail(GalleryErrors.GAL_ERR_UNSUPPORTED, 403)

        save_path = os.path.join(current_app.config['UPLOADS_DIR'], filename)
        if os.path.exists(save_path):
            return responce_fail(GalleryErrors.GAL_ERR_MEDIA_EXISTS, 403)
    ...
    if request.method == 'GET':
        ...
        if not os.path.exists(load_path):
            return responce_fail(GalleryErrors.GAL_ERR_MEDIA_NOT_EXIST, 403)
